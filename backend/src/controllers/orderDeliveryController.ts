...
export const createDelivery = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  const created_by_id = req.user?.id; // Get user ID from token
  ...
  try {
    ...
    const newDeliveryId = await OrderDelivery.create(newDeliveryData, created_by_id)
...
export const updateDelivery = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const { qty_return, total_amount, notes } = req.body
    const updateData: Partial<Pick<IOrderDelivery, 'qty_return' | 'total_amount' | 'notes'>> = {}
    ...
    if (notes !== undefined) updateData.notes = notes // Keep as string or null

    if (Object.keys(updateData).length === 0) {
      res.status(400).json({ message: 'No update data provided (allowed fields: qty_return, total_amount, notes)' })
      return
    }

    // Optional: Add role check? Can delivery boy update their own deliveries? Or only admin/staff?
    // const requestingUser = req.user;
    // const delivery = await OrderDelivery.findById(id); // Need to fetch to check delivery_boy_id
    // if (!delivery) { return res.status(404).json({ message: 'Delivery not found' }); }
    // if (requestingUser?.roleId === DELIVERY_BOY_ROLE_ID && requestingUser?.userId !== delivery.delivery_boy_id) { ... }
